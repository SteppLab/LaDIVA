function [all_pulses, this_pulse, f0] = DIVA_to_LeTalker_interface(ct,ta,pressure,voicing, restartSimulation) %[p, r] = LeTalker_run_continuously(p,c,N,Fs,ctvect,tavect,plvect)
...
%Date: 06.12.2013
%Modified by MCler Fall2015 to be compatible with DIVA
...
% % %     keep_track_tmp = 0;
    ...
    %x := (c - a) * (z - y) / (b - a) + y
    rescaledPressure = (pressure - -1) * (20000 - 2000) / (1 - -1)  + 2000; %a=-1, b=1; y = 2000 z=20000 
    %rescaledPressure = (pressure) * (20000 - 0) + 0; %##For now, min pressure = 7000; otherwise no sound
    rescaledVoicing = (voicing - -1) * (0 - .03) / (1 - -1) + .03;   %a=-1, b=1; y = .05 z=0 
    rescaledCT = (ct - -1) * (1 - 0) / (1 - -1) + 0;  %-1 to 1 vs 0 to 1 
...
%%%%%%%%%%
% This section is janky; just to rescale to approx what DIVA_synth is expecting in glottal pressure
 %The general case (when you have a value c between a and b and you want a value x between y and z), x is calculated as follows:
 %x := (c - a) * (z - y) / (b - a) + y
%rescaledPulses = (all_pulses - 0) * (2 - 0) / (4816 -0 ) + 0; %a=0, b=4816; y =0 z= 1   %max glottal pressure from permutations
rescaledPulses = (all_pulses - 0) * (3 - 0) / (10000 -0 ) + 0; %a=0, b=4816; y =0 z= 1   %max glottal pressure from permutations
%%%%%%%%%%%%%%

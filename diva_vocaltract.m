  ...
  if ~nargin,
      if ~DIVA_x.debug
          ...
          varargout{1}=struct(...
              ...
              'DefaultSilence',[repmat([-1,1],[10,1]);0.175,0.175;0.175,0.175;0.4,0.4;0.5,0.5],...
              'DefaultSound',[repmat([-1,1],[10,1]);-1,1;-1,1;-1,1;-1,1],...
              'BlockDiagonal',bsxfun(@eq,[ones(1,10),2,3,4,5],[ones(1,10),2,3,4,5]'),...
              'Plots_dim',{[{1:10},num2cell(1:10),{11,12,13,14}]},...
              'Plots_label',{[{'VocalTract'},arrayfun(@(n)sprintf('vt%d',n),1:10,'uni',0),{'CT_tension','TA_tension','pressure','voicing'}]} ),...
              'Output',[...
              struct(...
              'Name','Auditory',...
              'Dimensions',4,...
              'Range',[100,150;0,1000;0*500,3000;0*2000,4000],...
              'Scale',[100,500,1500,3000]',... % HW modified scaling here
              'Default',[0,200;0,1000;0*500,3000;0*2000,4000],...
              'DefaultSilence',[100,200;0,1000;0*500,3000;0*2000,4000],...
              'DefaultSound',[100,200;0,1000;500,3000;2000,4000],...
              'Plots_dim',{{2:4,1,2,3,4}},...
              'Plots_label',{{'Formants','F0','F1','F2','F3'}} ),...
              struct(...
              'Name','Somatosensory',...
              'Dimensions',8,...
              'Range',[repmat([-1,1],[6,1]);0,100;-1,1 ], ...% HRW 5201010 %repmat([-1,1],[8,1]),...
              'Scale',ones(8,1),...
              'Default',[repmat([-1,-.25],[6,1]);.75,1;.75,1],...
              'DefaultSilence',[repmat([-1,1],[6,1]);-1,-.5;-1,1],...
              'DefaultSound',[repmat([-1,-.25],[6,1]);.75,1;.75,1],...
...
if nargin<2, dodisp=0; end
  ...
  if ~DIVA_x.debug
      ...
      if nargout==1&&~dodisp
          y=diva_synth(x);
          %DIVA_x.params.Output(1).Scale(1,1) = 1; % HW added to remove scaling on F0 
          y=y./DIVA_x.params.Output(1).Scale;
      else
          [y,z,Outline]=diva_synth(x);
          y=y./DIVA_x.params.Output(1).Scale;
         % DIVA_x.params.Output(2).Scale(1,1) = 1;
          z=z./DIVA_x.params.Output(2).Scale;
      end
      %disp(['vocal tract x: ',num2str(x(11,1)),' y :',num2str(y(1,1))]);

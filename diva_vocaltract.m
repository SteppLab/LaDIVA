  ...
  elseif ischar(block)
      switch(lower(block)),
          case 'auditory'
              ...
              varargout{1} = varargout{1}(2:end); % don't include F0
          case 'somatosensory'
              % out{1} = aud, out{2} = som
              [out{1},out{2}]=diva_vocaltractcompute(varargin{:});
              varargout{1}=out{2}(1:6); % don't include pressure and voicing
          case 'voice' %% JI: hack in computation for voice only
              [a, s] = diva_vocaltractcompute(varargin{:});
              varargout{1} = [a(1); s(end-1:end)]; %fo (tension), pressure, voicing %HW
...
if nargin<2, dodisp=0; end
  ...
  if ~DIVA_x.debug
      ...
      if nargout==1&&~dodisp
          ...
          [y,z,Outline]=diva_synth(x); % JI: y = Aud, z = Som
          y=y./DIVA_x.params.Output(1).Scale;

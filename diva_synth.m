...
%extantArt = Art;
%load('fake_input_redux.mat')
%load('orig_a_artic.mat'); %from original DIVA
%Art(11:13,:) = fake(11:13,1:length(Art));
...
while time<(ndata+1)*dt;
    ...
    vt.pressure=FPV(2);  %originally: either 0 or 1
    vt.pressure0=vt.pressure>.01; %originally: either 0 or 1
    %vt.f0=100+20*FPV(1);
...
% %     synth.samplesperperiod=ceil(synth.fs/synth.f0);
    ...
    [all_pulses, new_pulse, vt.f0] = DIVA_to_LeTalker_interface(Art(11,min(ndata,1+t0)), .1, Art(12,min(ndata,1+t0)), Art(13,min(ndata,1+t0)),restartSimulation); %(.2,.2,7820, .01, restartSimulation);%(Art(11), .1, Art(7), Art(13)); %pass function CT activation, TA activation (doesn't change yet),  lung pressure -1 to 1, and voicing -1 to 1
    %[all_pulses, new_pulse, vt.f0] = LeTalker_run_continuously(fake(11,min(ndata,1+t0)), .1, fake(12,min(ndata,1+t0)), fake(13,min(ndata,1+t0)),restartSimulation); %(.2,.2,7820, .01, restartSimulation);%(Art(11), .1, Art(7), Art(13)); %pass function CT activation, TA activation (doesn't change yet),  lung pressure -1 to 1, and voicing -1 to 1

    synth.glottalsource = new_pulse'; 
    synth.samplesperperiod = length(synth.glottalsource); %%% ##MJC
    ...
    if release>0,%&&synth.pressure>.01,
        %u = synth.glottalsource; %###MJC added
        u = synth.glottalsource .* ( 1 + .05 * randn(synth.samplesperperiod,1) );

%         u = synth.voicing*1*.010*(synth.pressure+20*synth.pressurebuildup)*synth.glottalsource + ...
%           (1-synth.voicing)*1*.010*(synth.pressure+20*synth.pressurebuildup)*randn(synth.samplesperperiod,1);  %##comment out?
%       
...
%         end
        ...
        synth.pressure=synth.pressure/10; %##comment out?
    ...
    if numberofperiods>0,
         %u = synth.glottalsource; %figure(55); subplot(2,1,1); plot(u); xlabel('u before filter'); %##MJC
         u = synth.glottalsource .*(1+.05*randn(synth.samplesperperiod,1));
        %u=0.25*synth.modulation*synth.pressure*synth.glottalsource.*(1+.1*randn(synth.samplesperperiod,1)); % vocal tract filter
        %u=0.25*synth.pressure*synth.glottalsource.*(1+.01*randn(synth.samplesperperiod,1)); % vocal tract filter  %##comment out?
        %u=(synth.voicing*u+(1-synth.voicing)*.025*synth.pressure*randn(synth.samplesperperiod,1));  %##comment out?
        if minaf0>0&&minaf0<=k, u=minaf/k*u+(1-minaf/k)*.002*synth.pressure*randn(synth.samplesperperiod,1); end  %??
    ...
    v=(1-exp(-v))./(1+exp(-v)); % some kind of filter to trim large values
    ...
    % JI: I could incorporate these into the sample part?
...
%figure(9); subplot(2,1,1); plot(all_pulses,'r'); subplot(2,1,2); plot(s,'b');
save(['SynthCombined_' datestr(now, 'dd-mmm-yyyy_HH-MM-SS')], 's','all_pulses');
figure(66);
subplot(2,1,1);
xlabel('time (s)')
plot((1:length(all_pulses))/synth.fs, all_pulses); %ylim([0 2.5]);
title('Glottal cycles for /a/')
subplot(2,1,2);
plot((1:length(s))/synth.fs,s);
title('Sound wave of /a/')
xlabel('time (s)')
...
persistent vt fmfit voice;
if isempty(vt) % JI: called during gui initialization
    [filepath,filename]=fileparts(mfilename);
    load(fullfile(filepath,[filename,'.mat']),'vt','fmfit');
end
if isempty(voice) %% JI: add new persistant variable with voice parameters
     voice = load('divaVoiceSmooth.mat', 'fmfit');
end
% computes vocal tract configuration
idx=1:10; % JI: this does not include F0, pressure, and voicing
% fmfit is precomputed to deal only with these 10 parameters
...
Som=zeros(8,1);
if nargout>3 % called with 'explicit' (the actual things)
    [a,b,sc,af,d]=xy2ab(Outline);
    Som(1:6)=max(-1,min(1, -tanh(1*sc) ));
    Som(7:8)=Art(end-1:end); % JI: somehow use Art(end-2) to update these
    % JI: call the real letalker here, this section uses the real thing
    %         [a,b,sc]=xy2ab(Outline);
end
% computes auditory/somatosensory output (through previously computed forward fit)
% JI: this part uses the precomputed models (for speed up?)
Aud=zeros(4,1);
if ~isempty(fmfit) % explicit: call the true models, not the estimates % JI: call the precomputed Letalker Forward Model...
    tmpAudOut = fmLetalker(voice, Art(end-2:end)');
    Aud(1) = tmpAudOut(1);
    dx=bsxfun(@minus,Art(idx)',fmfit.mu); % JI: formant fit transformation
    p=-sum((dx*fmfit.iSigma).*dx,2)/2; % JI: some kind of 2d gaussian thing
...
if ~isempty(fmfit)&&nargout>1&&nargout<=3,
    Som(1:6)=fmfit.beta_som*px(:); % JI: somatosensory update
    % JI: use above fmLetalker and scale the outputs down to -1/1
    Som(7:8) = tmpAudOut(2:3); % JI: I scaled this down already
end
end

% JI: forward model estimate for LeTalker
function testoutput = fmLetalker(voice, testinput)

MU = voice.fmfit.mu;
iSigma = voice.fmfit.iSigma;
P = voice.fmfit.p;
B_voice = voice.fmfit.beta_voice';

dx=bsxfun(@minus,testinput,MU); % HW: a normalizing function 
p=-sum((dx*iSigma).*dx,2)/2;
p=P.*exp(p-max(p));
p=p/sum(p);
px=p*[testinput,1];
testoutput=px(:)'*B_voice;

%%% JI: TO DO
% for now scale the outputs back down to -1 to 1 (except for F0)
% scaled = (x - min) / (max-min) * 2 - 1

% if the outputs are negative, do some janky stuff so that they're close to
% 0, but not really
if testoutput(1) < 0
    testoutput(1) = testoutput(1) / 1000;
end
if testoutput(2) < 0
    testoutput(2) = -log(-testoutput(2) + 1) / 10; 
% elseif testoutput(2) > 0
%     testoutput(2) = log(testoutput(2) + 1);
end
if testoutput(3) < 0
    testoutput(3) = -log(-testoutput(3) + 1) / 4; 
% elseif testoutput(3) > 0
%     testoutput(3) = log(testoutput(3) + 1);
end

testoutput(2) = testoutput(2) / 2300;% * 2 - 1;
testoutput(3) = testoutput(3) / 10;% * 2 - 1;
end
